...
curve_v1_lp_data = {}
for filename in [
    r"C:\Users\PC\Projects\dex_arb\arbitrum\degen_scripts\arbitrum_lps_curvev1_registry.json"
]:
    with open(filename) as file:
        pool_data = ujson.load(file)
    for pool in pool_data:
...
all_1inch_token = set(oneinch_token_data.keys())
...
for pool in curve_v1_lp_data.values():
    if pool.get("underlying_coin_addresses"):
        ...
        for token_address_pair in itertools.combinations(
            ...
            for token in oneinch_token_data.values():
                if token.get("token"):
                    if token in token_address_pair:
                        G.add_edge(
                            *token_address_pair,
                            token,
                            lp_address=pool["pool_address"],
...
for token in oneinch_token_data.values():
    if token == token.get("token"):
        all_tokens_with_pair_pool = list(G.neighbors(token))
        print(f"Found {len(all_tokens_with_pair_pool)} tokens with a WETH pair")
        ...
        filtered_tokens = [
            token for token in all_tokens_with_pair_pool if G.degree(token) > 1
        ]
        ...
        for token_a, token_b in itertools.combinations(filtered_tokens, 2):
            ...
            outside_pools_tokenA = [
                edge["lp_address"]
                for edge in G.get_edge_data(token_a, token).values()
                if edge["pool_type"] in ["UniswapV2", "UniswapV3", "1inch_Aggregator"]
            ]

            # find tokenB/WETH pools (Uniswap only)
            outside_pools_tokenB = [
                edge["lp_address"]
                for edge in G.get_edge_data(token_b, token).values()
                if edge["pool_type"] in ["UniswapV2", "UniswapV3", "1inch_Aggregator"]
            ]
            ...
            for pool_addresses in itertools.product(
                ...
                for pool_address in pool_addresses:
                    if pool_address in all_uni_v2_pools:
                        pool_info = v2_lp_data.get(pool_address)
                        if token in (pool_info.get("token0") , pool_info.get(["token1"])):
                            pool = {
                                pool_address: v2_lp_data[pool_address],
                                "1inch_connected token": token
                                }
                        else:
                        ...
                        if token in (pool_info.get("token0") , pool_info.get(["token1"])):
                            pool = {
                                pool_address: v3_lp_data[pool_address],
                                "1inch_connected token": token
                                }
                        else:
                            pool_info = {pool_address: v2_lp_data[pool_address]}
                        ...
                        if token in (pool_info.get("coin_addresses"), pool_info.get("underlying_coin_addresses")):
                            pool = {
